...
class PLibDataLoader:
    ...
    def __init__(self, cfg, device=None):
        ...
        if weight_cfg:
            ...
            if method == "vis":
                ...
                self.get_weight = lambda vis: torch.tensor(1.0, device=device)
            ...
            assert self._n_photon is not None, (
                "Key n_photon is missing from the config file! Double check the input."
            )
        ...
        assert self._n_pmt > 0, (
            "out_features of the model doesn't agree with the actual n_pmt config"
        )
        ...
        if os.path.isfile(pmt_coords_file):
            self.pmt_coords = (
                torch.from_numpy(np.loadtxt(pmt_coords_file, delimiter=","))
                .float()
                .to(device)
            )
        else:
        ...
        if not self._lazy_load:
            ...
            vis_adapt = torch.cat(
                [
                    vis.view(vis.shape[0], self._n_pmt, -1).sum(-1),
                    vis.view(vis.shape[0], -1),
                ],
                dim=1,
            )
            ...
            target = None
        self._cache = dict(
            position=pos, value=vis_adapt, weight=w, target=target, tof=self.tof
        )
    ...
    def correct_tof(self, vis, tof):
        ...
        t_shift = (
            (tof / 0.1).unsqueeze(-1).long().to(vis.device)
        )  # hardcoded 100ps per bin for 100ns window
        ...
        vis_shifted[valid_mask] = vis[
            v_coords[valid_mask], n_coords[valid_mask], source_t_idx[valid_mask]
        ]
        """
    ...
    def __iter__(self):
        ...
        if self._batch_mode:
            ...
            for b in range(len(self)):
                ...
                if not self._lazy_load:
                    output = dict(
                        ...
                        if not self.remove_tof
                        else self.correct_tof(
                            self._cache["value"][vox_ids], self._cache["tof"][vox_ids]
                        ),
            ...
            if not self._lazy_load:
                ...
                vis_adapt = torch.cat(
                    [
                        out_vis.view(vis.shape[0], self._n_pmt, -1).sum(-1),
                        out_vis.view(out_vis.shape[0], -1),
                    ],
                    dim=1,
                )
                output = dict(
