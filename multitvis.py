...
class MultiSirenTV(torch.nn.Module):
    ...
    def configure(self, cfg : dict):
        ...
        if len(weights) and len(weight_id):
            ...
            for i,w in enumerate(weights):
                print(f'[MultiVis] adding {i+1}/{len(weights)} SirenTVis instances...\n')
                self.add_model(SirenTVis.load(w))
    ...
    def visibility2(self, x):
        ...
        vis = torch.zeros(size=(x.shape[0],self.n_outs),dtype=torch.float32,device=self.device)
